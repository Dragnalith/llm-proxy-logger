#!/usr/bin/env python3
"""Minimal script to make requests to Anthropic Message API."""
...
def make_request(url: str, model: str, message: str, use_provider_key: bool = False, system: str = "You are AI Chat made to test llm-proxy-logger project. Your answer are as short as possible") -> str:
    """
    Make a request to Anthropic Message API.
    
    Args:
        url: Base URL (e.g., http://localhost:4000)
        model: Model slug
        message: User message
        use_provider_key: If True, use ANTHROPIC_API_KEY; otherwise use LITELLM_MASTER_KEY
...
def main():
    parser = argparse.ArgumentParser(description="Make request to Anthropic Message API")
    ...
    parser.add_argument("--provider-key", action="store_true", help="Use ANTHROPIC_API_KEY instead of LITELLM_MASTER_KEY")
